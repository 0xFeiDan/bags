...
FREE_REQUESTS_PER_SECOND_LIMIT = 3
FREE_DAILY_REQUEST_LIMIT = 2_000
FREE_MAX_REQUESTS_PER_RUN = 3
FREE_MIN_SYNC_INTERVAL_SECONDS = 900
FREE_DAILY_REQUEST_BUDGET = 1_800
...
def configure_source(
    ...
    # Refresh persisted caps whenever an operator saves the source. Environment
    # settings may tighten the free plan, while provider headers can tighten a
    # specific day's observed quota further during a run.
    source.requests_per_second_limit = requests_per_second_limit
    source.daily_request_limit = daily_request_limit
    source.max_requests_per_run = max(1, min(settings.zerion_max_requests_per_run, FREE_MAX_REQUESTS_PER_RUN))
    source.min_sync_interval_seconds = max(settings.zerion_min_sync_interval_seconds, FREE_MIN_SYNC_INTERVAL_SECONDS)
    source.daily_request_budget = daily_request_budget
