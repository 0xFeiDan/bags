...
class Settings(BaseSettings):
    ...
    # key. Defaults follow the user's current free-plan quota; provider
    # response headers can tighten them further during a run.
    ...
    zerion_requests_per_second_limit: int = 3
    zerion_daily_request_limit: int = 2_000
    zerion_max_requests_per_run: int = 3
    zerion_min_sync_interval_seconds: int = 900
    # Keep 10% of the 2,000-request daily quota as a diagnostic reserve.
    zerion_daily_request_budget: int = 1_800
